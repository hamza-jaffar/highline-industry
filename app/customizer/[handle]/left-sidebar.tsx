...
const CustomizerLeftSidebar = ({ isMobile }: { isMobile?: boolean }) => {
    ...
    return (
        <div className="flex w-full h-full bg-[#f0f1f2] border-r border-black/5 overflow-hidden shrink-0">
                        ...
                        <div className="flex bg-gray-100 p-1 rounded-xl w-full">
                            {NAV_ITEMS.filter(i => i.id !== 'shortcuts').map((item) => (
                                <button
                                    key={item.id}
                                    onClick={() => dispatch(setTab(item.id))}
                                    className={`flex-1 py-2.5 px-4 flex items-center justify-center gap-2 rounded-lg transition-all text-xs font-black ${
                                        activeTab === item.id 
