...
export default function CollectionsAdminPage() {
  ...
  const [reverse, setReverse] = useState(searchParams.get("reverse") ? searchParams.get("reverse") === "true" : true);
  ...
  const fetchCollections = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    const result = await getAdminCollections(per_page, cursor, debouncedSearch, sortKey, reverse);
    if (result.success) {
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            ...
            {isPerPageOpen && (
              <>
                <div className="fixed inset-0 z-40" onClick={() => setIsPerPageOpen(false)} />
                <div className="absolute top-full right-0 mt-2 w-32 bg-white border border-black/10 rounded-xl shadow-xl z-50 overflow-hidden">
                  {[10, 20, 50].map((val) => (
                    <button
                      key={val}
                      onClick={() => {
                  ...
                  {isLoading ? (
                    Array.from({ length: per_page }).map((_, i) => (
                      <tr key={`skeleton-${i}`} className="animate-pulse">
                        ...
                        <td className="px-6 py-4">
                          <div className="h-4 w-16 bg-black/20 rounded mx-auto" />
                        </td>
                        <td className="px-6 py-4">
