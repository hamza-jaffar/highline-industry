    ...
    <div className="fixed inset-0 z-[100] h-screen flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-black/10 overflow-hidden animate-in zoom-in-95 fade-in duration-300">
        ...
        <div className="px-6 py-6 border-b border-black/5 flex items-start gap-4">
          <div className={`mt-1 p-2 rounded-xl flex-shrink-0 ${variant === "danger" ? "bg-red-50 text-red-500 border border-red-100" : "bg-blue-50 text-blue-500 border border-blue-100"
            }`}>
        ...
        <div className="px-6 py-4 bg-[#fafafa] flex items-center justify-end gap-3">
          <button
            onClick={onCancel}
            disabled={isLoading}
            className="px-4 py-2 text-sm font-semibold text-[#737373] hover:text-[#111] transition-colors disabled:opacity-50"
          >
            {cancelLabel}
          </button>
          <button
            onClick={onConfirm}
            disabled={isLoading}
            className={`px-6 py-2 rounded-xl text-sm font-semibold transition-all shadow-sm flex items-center gap-2 ${variant === "danger"
